...
last_alarm_time = 0          # Tracks when the last alarm event happened
BUZZER_DURATION = 0.5        # How long the buzzer sounds per trigger (seconds)
BUZZER_COOLDOWN = 4.0        # How long the buzzer MUST rest after sounding (seconds)
actuators_active = False     # Tracking flag for physical state
...
try:
    while True:
        ...
        # Automatically turn off the buzzer/LED after BUZZER_DURATION has elapsed
        if actuators_active and (now - last_alarm_time > BUZZER_DURATION):
            led_bulb.off()
            buzzer.off()
            actuators_active = False
            # Clear old buffer frames that built up while the buzzer was making noise
            try:
        ...
        if aoi_active or audio_model_triggered or spike_triggered:
            ...
            # Check if the buzzer has rested long enough to sound again
            ...
            if buzzer_allowed and not actuators_active:
                ...
                last_alarm_time = now  # Update timer reference
                buzzer_status_text = "🔊 SOUNDING & SAVING DATA"
            else:
                buzzer_status_text = "💤 RESTING (Buzzer Cooldown active) | SAVING DATA CONTINUOUSLY"

            # Render bounding box overlays on output frame
            ...
            # Continuous logging payload - never stops execution
            print("\n" + "="*70)
            print(f"🚨 EVENT CAPTURED - [{buzzer_status_text}]")
            print("="*70)
            print(f"Time                        : [{current_time_str}]")
            print(f"PIR Status                  : {'ACTIVE' if pir_active else 'INACTIVE'}")
            ...
            # Always save the photo data immediately to disk
            ...
            # Standard console ticker line
            cooldown_left = max(0.0, BUZZER_COOLDOWN - (now - last_alarm_time))
            print(f"Scanning Space... [PIR: {int(pir_active)} | Spike: {db_interval_change:.1f}dB | Vision: {vision_score:.2f} | Buzzer Cooldown Rest: {cooldown_left:.1f}s]", end="\r")
